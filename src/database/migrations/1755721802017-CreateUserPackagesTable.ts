...
export class CreateUserPackagesTable1755721802017
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const existingForeignKey = table.foreignKeys.find(
      (fk) =>
        fk.columnNames.includes('package_id') &&
        fk.referencedTableName === 'packages',
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.dropIndex(
      'user_packages',
      'idx_user_package_active_subscription',
    );
    await queryRunner.dropIndex(
      'user_packages',
      'idx_user_package_stripe_subscription',
    );
