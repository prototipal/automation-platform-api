...
export class AddFreePackageForExistingUsers1755804832035
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!freePackageResult || freePackageResult.length === 0) {
      throw new Error(
        'Free package not found. Please ensure packages are created first.',
      );
    }
    ...
    if (usersWithoutPackages && usersWithoutPackages.length > 0) {
      console.log(
        `Found ${usersWithoutPackages.length} users without packages. Adding free packages...`,
      );
      ...
      console.log(
        `Successfully added free packages for ${usersWithoutPackages.length} users.`,
      );
    } else {
