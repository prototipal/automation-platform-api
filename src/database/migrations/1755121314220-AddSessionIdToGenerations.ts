...
export class AddSessionIdToGenerations1755121314220 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!generationsTableExists) {
            ...
            const sessionIdColumnExists = await queryRunner.hasColumn('generations', 'session_id');
            
            if (!sessionIdColumnExists) {
                await queryRunner.addColumn('generations', new TableColumn({
                    name: 'session_id',
                    type: 'int',
                ...
                await queryRunner.changeColumn('generations', 'session_id', new TableColumn({
                    name: 'session_id',
                    type: 'int',
                    isNullable: false,
                }));
                ...
                try {
                    ...
                    if (error.message && error.message.includes('already exists')) {
                        console.log('Index idx_generation_session_id already exists, skipping...');
                    } else {
                        throw error;
                    }
                }
                ...
                try {
                    ...
                    if (error.message && error.message.includes('already exists')) {
                        console.log('Index idx_generation_session_lookup already exists, skipping...');
                    } else {
                        throw error;
                    }
                }
                ...
                try {
                    ...
                    if (error.message && error.message.includes('already exists')) {
                        console.log('Index idx_generation_user_session already exists, skipping...');
                    } else {
                        throw error;
                    }
                }
                ...
                try {
                    ...
                    if (error.message && error.message.includes('already exists')) {
                        console.log('Index idx_generation_session_status already exists, skipping...');
                    } else {
                        throw error;
                    }
                }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (generationsTableExists) {
            // Drop indexes first
            await queryRunner.dropIndex('generations', 'idx_generation_session_status');
            await queryRunner.dropIndex('generations', 'idx_generation_user_session');
            await queryRunner.dropIndex('generations', 'idx_generation_session_lookup');
