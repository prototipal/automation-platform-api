...
export class AddMainCategoryIdToCategories1754827753702 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const hasIndex = table?.indices.find(index => index.name === 'IDX_categories_main_category_id');
        if (!hasIndex) {
        ...
        const hasForeignKey = table?.foreignKeys.find(fk => fk.name === 'FK_categories_main_category');
        if (!hasForeignKey) {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const table = await queryRunner.getTable('categories');
        const foreignKey = table?.foreignKeys.find(fk => fk.name === 'FK_categories_main_category');
        if (foreignKey) {
            await queryRunner.dropForeignKey('categories', 'FK_categories_main_category');
        }

        // Check if index exists before dropping it
        const hasIndex = table?.indices.find(index => index.name === 'IDX_categories_main_category_id');
        if (hasIndex) {
            await queryRunner.dropIndex('categories', 'IDX_categories_main_category_id');
        }
