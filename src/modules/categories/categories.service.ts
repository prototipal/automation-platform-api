...
export class CategoriesService {
  ...
  async findAll(queryDto: QueryCategoryDto): Promise<PaginatedResult<CategoryResponseDto>> {
  ...
  async update(id: string, updateCategoryDto: UpdateCategoryDto): Promise<CategoryResponseDto> {
    ...
    if (updateCategoryDto.name && updateCategoryDto.name !== existingCategory.name) {
      const categoryWithName = await this.categoriesRepository.findByName(updateCategoryDto.name);
      if (categoryWithName) {
  ...
  async remove(id: string): Promise<void> {
    ...
    if (category.templates && category.templates.length > 0) {
      throw new BadRequestException(`Cannot delete category with ${category.templates.length} associated templates. Please reassign or delete the templates first.`);
    }
  ...
  async getStats(): Promise<{ total: number; byType: Record<string, number>; withTemplates: number }> {
    ...
    const byType = allCategories.data.reduce((acc, category) => {
  ...
  async findOrCreate(name: string, link?: string, type: 'photo' | 'video' = 'photo'): Promise<CategoryResponseDto> {
    const category = await this.categoriesRepository.findOrCreate(name, link, type);
    return plainToInstance(CategoryResponseDto, category);
  }
