...
export class CreditManagementService {
  ...
  async getCreditBalance(userId: string): Promise<CreditBalance | null> {
    try {
      ...
      this.logger.error(
        `Error getting credit balance for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async deductCredits(
    ...
    try {
      this.logger.log(
        `Deducting ${request.amount} credits from user ${request.user_id}`,
      );

      const result =
        await this.userCreditsRepository.deductCreditsAtomic(request);

      if (!result.success) {
        this.logger.warn(
          `Credit deduction failed for user ${request.user_id}: ${result.error}`,
        );
        return {
          success: false,
          deducted_amount: 0,
      ...
      const creditTypeUsed =
        updatedCredit.metadata?.last_deduction_type === 'api'
          ? CreditType.API
      ...
      this.eventEmitter.emit('credit.deducted', {
        ...
        remaining_playground_credits:
          updatedCredit.available_playground_credits,
      ...
      this.logger.error(
        `Error deducting credits for user ${request.user_id}:`,
        error,
      );
      return {
        success: false,
  ...
  async refillCredits(request: CreditRefillRequest): Promise<void> {
    try {
      ...
      const updatedCredit =
        await this.userCreditsRepository.refillCredits(request);
      ...
      this.logger.error(
        `Error refilling credits for user ${request.user_id}:`,
        error,
      );
      throw error;
    }
  ...
  async resetPlaygroundCredits(request: CreditResetRequest): Promise<void> {
    try {
      ...
      const updatedCredit =
        await this.userCreditsRepository.resetPlaygroundCredits(request);
      ...
      this.logger.error(
        `Error resetting playground credits for user ${request.user_id}:`,
        error,
      );
      throw error;
    }
  ...
  async handleSubscriptionPeriodStart(
    ...
    try {
      ...
      this.logger.log(
        `📦 Package: ${packageName} (${packageId}), Credits: ${monthlyCredits}`,
      );
      this.logger.log(
      ...
      // Set next reset date
      await this.userCreditsRepository.setPlaygroundCreditsNextReset(
        userId,
        periodEnd,
      );
      ...
      this.logger.error(
        `Error handling subscription period start for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async handleSubscriptionCancellation(
    ...
    try {
      this.logger.log(
        `Handling subscription cancellation for user ${userId}, package ${packageId}`,
      );
      ...
      await this.resetPlaygroundCredits({
        ...
        source: CreditSource.SUBSCRIPTION_RESET,
        description:
          'Playground credits reset due to subscription cancellation',
        metadata: {
          package_id: packageId,
      ...
      this.logger.log(
        `Successfully handled subscription cancellation for user ${userId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error handling subscription cancellation for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async hasSufficientCredits(
    ...
    try {
      ...
      this.logger.error(
        `Error checking credit sufficiency for user ${userId}:`,
        error,
      );
      return false;
    }
  ...
  async getCreditUsageReport(
    userId: string,
  ): Promise<CreditUsageReport | null> {
    try {
      ...
      this.logger.error(
        `Error getting credit usage report for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async createUserCredits(
    ...
    try {
      ...
      this.logger.error(
        `Error creating credit record for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async migrateUserCredits(userId: string): Promise<void> {
    try {
      const result =
        await this.userCreditsRepository.migrateExistingBalance(userId);
  ...
  async processScheduledPlaygroundResets(): Promise<void> {
    this.logger.log(
      'Scheduled playground resets are disabled - using webhook-based credit management',
    );
